...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data .Majacoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming .Majacoin
    // Mac: ~/Library/Application Support .Majacoin
    // Unix: ~/.Majacoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / ".Majacoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Majacoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Majacoin.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", ".Majacoind.pid"));
